...
const Instructions = () => {
    ...
    const swipeAction = () =>{
        ...
        if ((pivotCoordinates.current.x - currentCoordinates.current.x).toFixed(3)  > 0.20 && currentIndex < 4 && !cooldownRef.current) {
          const elements = document.getElementsByClassName("rec-arrow-right");
        ...
        } else if ((pivotCoordinates.current.x - currentCoordinates.current.x).toFixed(3) < -0.20 && currentIndex > 0 && !cooldownRef.current) {
          const elements = document.getElementsByClassName("rec-arrow-left");
          const elementArray = Array.from(elements);
          if(elementArray){
            elementArray.forEach((element) => {
                    element.click();
    ...
    useEffect(() => {
       ...
       if(pivotStateRef.current){
        ...
        if(predictionData.prediction !== 'rock'){
            if(verticalDiff < -30 && Math.abs(diff) < 0.1){
                console.log("Scrolling")
                window.scrollBy(0,window.innerHeight/30);
            }else if(verticalDiff > 30 && Math.abs(diff) < 0.1){
                console.log("Scrolling")
                window.scrollBy(0, -window.innerHeight/30);
